...
# Load the custom spaCy model for resume keyword matching
nlp = spacy.load("en_Resume_Matching_Keywords")

# Load the Sentence Transformer model
model_path = "C:\\Users\\acer\\Downloads\\flask_resume_similarity\\flask_resume_similarity\\Matching-job-descriptions-and-resumes\\msmarco-distilbert-base-tas-b-final"
...
def process_resumes_and_jd(resume_files, jd_text):
    ...
    for resume_file in resume_files:
        ...
        for entity in labels_to_extract:
            match_data[entity] = ', '.join(resume_entities.get(entity, []))  # Join multiple values with comma if necessary
...
def upload_resume():
    ...
    if request.method == 'POST':
        ...
        results = process_resumes_and_jd(resume_files, jd_text)

    return render_template('resume_matching.html', results=results)
